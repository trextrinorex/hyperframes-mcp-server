...
const HYPERFRAMES_BIN = process.env.HYPERFRAMES_BIN || '/app/node_modules/.bin/hyperframes';
...
async function runHyperframes(args, cwd = ROOT) {
  const { stdout, stderr } = await execFileAsync(HYPERFRAMES_BIN, args, {
    cwd, env: { ...process.env, CI: '1', PUPPETEER_EXECUTABLE_PATH: process.env.PUPPETEER_EXECUTABLE_PATH || '/usr/bin/chromium' },
    timeout: COMMAND_TIMEOUT, maxBuffer: MAX_OUTPUT
  });
